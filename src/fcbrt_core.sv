...
module fcbrt_core(
    ...
    output logic [C_MANT_FP64+4:0] mant_up_o,
    output logic [C_EXP_FP64-1:0] exp_bias_o,
    output logic [C_EXP_FP64-1:0] exp_bias_up_o,
    ...
    logic [C_INIT_OW-1:0] mant_init_o_Reg;
    logic [2*(C_INIT_OW-1):0] init_sq_o_Reg;
    logic [3*(C_INIT_OW-1):0] init_cb_o_Reg;
    logic [C_MANT_FP64+3:0] mant_norm_init_Reg;
    ...
    // init bundle pipeline
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (~rst_ni) begin
            init_stage_valid <= 1'b0;
            mant_init_o_Reg  <= '0;
            init_sq_o_Reg    <= '0;
            init_cb_o_Reg    <= '0;
            mant_norm_init_Reg <= '0;
        end else begin
            init_stage_valid <= core_start;
            if (core_start) begin
                mant_init_o_Reg   <= mant_init_o;
                init_sq_o_Reg     <= init_sq_o;
                init_cb_o_Reg     <= init_cb_o;
                mant_norm_init_Reg <= mant_norm_i;
            end
        end
    end
    ...
    logic [C_MANT_FP64+2:0] S_hi;
    logic [C_MANT_FP64+2:0] SA1; // U1.54, 取 S[56:2] + 1
    logic [C_MANT_FP64+2:0] SA2; // U1.54, 取 S[56:2] + 2

    assign S = {mant_init_o_Reg, {(C_MANT_FP64+5-C_INIT_OW){1'b0}}};
    assign SM = {(mant_init_o_Reg - {{(C_INIT_OW-1){1'b0}}, 1'b1}), {(C_MANT_FP64+5-C_INIT_OW){1'b0}}};
    assign S_hi = S[C_MANT_FP64+4:2];

    assign SA1 = {(mant_init_o_Reg + {{(C_INIT_OW-1){1'b0}}, 1'b1}),
                {(C_MANT_FP64+3-C_INIT_OW){1'b0}}};

    assign SA2 = {(mant_init_o_Reg + {{(C_INIT_OW-2){1'b0}}, 2'b10}),
                {(C_MANT_FP64+3-C_INIT_OW){1'b0}}};

    // S_Square Init
    logic [2*(C_MANT_FP64+4)+4:0] S_Square; // U1.112 更改
    assign S_Square = {4'b0000, init_sq_o_Reg, {(2*(C_MANT_FP64+4)-2*(C_INIT_OW-1)){1'b0}}};

    // Residual Init
    logic [2*(C_MANT_FP64+4)+4:0] Residual; // Q5.112 
    logic [C_MANT_FP64+3:0] Residual_Init; // Q1.55
    
    assign Residual_Init = mant_norm_init_Reg - {init_cb_o_Reg, 31'b0}; // X-S^3
    ...
    logic [C_MANT_FP64+2:0] SA1_init_N; // U1.54
    logic [C_MANT_FP64+2:0] SA2_init_N; // U1.54
    logic [2*(C_MANT_FP64+4)+4:0] S_Square_init_N; // U1.112 更改
    logic [2*(C_MANT_FP64+4)+4:0] Residual_init_N; // Q5.112

    logic [C_MANT_FP64+4:0] S_Reg;   // U1.56
    logic [C_MANT_FP64+4:0] SM_Reg;  // U1.56
    logic [C_MANT_FP64+2:0] SA1_Reg; // U1.54
    logic [C_MANT_FP64+2:0] SA2_Reg; // U1.54

    assign S_init_N = S;
    assign SM_init_N = SM;
    assign SA1_init_N = SA1;
    assign SA2_init_N = SA2;
    assign S_Square_init_N = S_Square;
    assign Residual_init_N = Residual;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (~rst_ni) begin
            Fsm_enable <= '0;
        end else if (init_stage_valid) begin
            Fsm_enable <= 1'b1;
    ...
    // S/SM/SA Ssquare Residual r4 path 

    logic [C_MANT_FP64+4:0] S_o; 
    logic [C_MANT_FP64+4:0] SM_o;
    logic [C_MANT_FP64+2:0] SA1_o;
    logic [C_MANT_FP64+2:0] SA2_o;

    logic [2*(C_MANT_FP64+4)+4:0] Sq_c_o;
    logic [2*(C_MANT_FP64+4)+4:0] Sq_s_o;
    ...
    logic [2*(C_MANT_FP64+4)+4:0] S_Square_c_Reg; // r64迭代后的Sq冗余表示
    logic [2*(C_MANT_FP64+4)+4:0] S_Square_s_Reg;
    ...
    // sticky / sign（将 117 位加法拆成两拍）
    localparam int RESIDUAL_LO_W = 58;
    localparam int RESIDUAL_HI_W = 59;

    logic sticky;
    logic [RESIDUAL_LO_W:0]   Residual_fix_lo_full;
    logic [RESIDUAL_HI_W-1:0] Residual_c_hi_Reg;
    logic [RESIDUAL_HI_W-1:0] Residual_s_hi_Reg;
    logic                     Residual_fix_carry_mid_Reg;
    logic                     Residual_fix_lo_nonzero_Reg;
    logic [RESIDUAL_HI_W-1:0] Residual_fix_hi;
    logic                     Residual_fix_sign_Reg;
    logic                     sticky_Reg;

    assign Residual_fix_lo_full = {1'b0, Residual_c_Reg[RESIDUAL_LO_W-1:0]} +
                                  {1'b0, Residual_s_Reg[RESIDUAL_LO_W-1:0]};

    assign Residual_fix_hi = Residual_c_hi_Reg + Residual_s_hi_Reg +
                             {{(RESIDUAL_HI_W-1){1'b0}}, Residual_fix_carry_mid_Reg};

    // 第一拍：计算低 58 位并寄存中间进位，保存高 59 位操作数
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (~rst_ni) begin
            Residual_c_hi_Reg            <= '0;
            Residual_s_hi_Reg            <= '0;
            Residual_fix_carry_mid_Reg   <= 1'b0;
            Residual_fix_lo_nonzero_Reg  <= 1'b0;
        end else begin
            Residual_fix_carry_mid_Reg   <= Residual_fix_lo_full[RESIDUAL_LO_W];
            Residual_fix_lo_nonzero_Reg  <= |Residual_fix_lo_full[RESIDUAL_LO_W-1:0];
            Residual_c_hi_Reg            <= Residual_c_Reg[2*(C_MANT_FP64+4)+4:RESIDUAL_LO_W];
            Residual_s_hi_Reg            <= Residual_s_Reg[2*(C_MANT_FP64+4)+4:RESIDUAL_LO_W];
        end
    end

    // 第二拍：计算高 59 位，直接生成 sign 和 sticky
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (~rst_ni) begin
            Residual_fix_sign_Reg <= 1'b0;
            sticky_Reg            <= 1'b0;
        end else begin
            Residual_fix_sign_Reg <= Residual_fix_hi[RESIDUAL_HI_W-1];
            sticky_Reg            <= (|Residual_fix_hi) | Residual_fix_lo_nonzero_Reg;
        end
    end
    ...
    fcbrt_S_SM_SA_r4 u_fcbrt_S_SM_SA_r4(
        .S_i        	( S_Reg        ),
        .SM_i       	( SM_Reg       ),
        .SA1_i      	( SA1_Reg      ),
        .SA2_i      	( SA2_Reg      ),
    ...
    logic [C_EXP_FP64-1:0] exp_bias_nonc_p1;
    ...
    assign exp_bias_nonc_p1 = exp_bias_nonc + {{(C_EXP_FP64-1){1'b0}}, 1'b1};

    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (~rst_ni) begin
            exp_bias_nonc_reg <= '0;
            exp_bias_nonc_p1_reg <= '0;
        end else if (cycle_cnt == Final_cycle) begin
            exp_bias_nonc_reg <= exp_bias_nonc;
            exp_bias_nonc_p1_reg <= exp_bias_nonc_p1;
        end else begin
            exp_bias_nonc_reg <= exp_bias_nonc_reg;
            exp_bias_nonc_p1_reg <= exp_bias_nonc_p1_reg;
        end
    end
    ...
    // 寄存最后一轮迭代的选择结果
    logic signed [2:0] s_last_Reg;

    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (~rst_ni) begin
            s_last_Reg <= '0;
        end else if ((cycle_cnt == Final_cycle) && Fsm_enable) begin
            s_last_Reg <= s_sel_i;
        end
    end

    // 输出信号
    assign start_o = start_d2;
    assign special_case_o = special_case_i;
    assign sticky_o = sticky;
    assign mant_o = (Residual_fix_sign_Reg)?SM_Reg:S_Reg;
    assign exp_bias_o = exp_bias_nonc_reg;
    assign exp_bias_up_o = exp_bias_nonc_p1_reg;

    // mant_up_o 选择
    always_comb begin
    case (s_last_Reg)
        3'sd2, 3'sd1: begin
            mant_up_o = (Residual_fix_sign_Reg) ? {SA2_Reg, SM_Reg[1:0]}
                                                : {SA2_Reg, S_Reg[1:0]};
        end

        3'sd0: begin
            mant_up_o = (Residual_fix_sign_Reg) ? {SA1_Reg, SM_Reg[1:0]}
                                                : {SA2_Reg, S_Reg[1:0]};
        end
